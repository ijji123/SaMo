...
class Player():
    def __init__(self, uart=None, busy_pin=None, config=True, volume=0.5):
        ...
        self.t0 = clock()
        if uart is None:
            self.uart = serial.Serial("/dev/ttyS0", 9600) # UART on 
        else:
            self.uart = uart
    ...
    def playing(self):
        if self.busy_pin is not None:
            self.awaitplay()
            return GPIO.input(self.busy_pin) == 0
        else:
